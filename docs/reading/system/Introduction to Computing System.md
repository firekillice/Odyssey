...
* 中断的开始，是在fetch节拍之前，Store result节拍之后发生的，也就是说，中断必须以一个指令为最小切换单元
* 在fetch节拍之前，判定中断信号
#### 键盘的数据的处理
* 放入到键盘的寄存器中
* 通知cpu数据读取
...
#### 通信方式
* Memory-mapped I/O(MMIO)，和主存统一的地址空间
* Port-mapped I/O (PMIO)， 内存和IO设备为各自的地址空间，需要执行专门的IO操作才可以访问; 为了实现地址空间的隔离，要么在CPU物理接口上增加一个I/O引脚，要么增加一条专用的I/O总线。由于I/O地址空间与内存地址空间是隔离的，所以有时将PMIO称为被隔离的IO(Isolated I/O)。
* 上面两个都不影响DMA，因为DMA是绕过CPU的内存对设备的通信；而上面两个其实都是CPU对设备的访问方式
* 不要与MMAP混淆，这个是为了减少系统调用的将用户和block缓存的直接打通，是一种用户与DMA的直接接触
